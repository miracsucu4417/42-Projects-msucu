...
// Test fonksiyonu: dosyaya ft_putendl_fd ile yaz, içeriği kontrol et
static int _run_putendl_fd_test_case(const char *string_to_write, const char *expected_content, const char *test_name) {
    ...
    ft_putendl_fd((char *)string_to_write, fd);
...
#define RUN_PUTENDL_FD_TEST(desc, str, expected) \
    total_tests++; \
    if (_run_putendl_fd_test_case(str, expected, desc)) { \
        passed_tests++; \
    }
...
int main(void) {
    ...
    RUN_PUTENDL_FD_TEST("Write normal string with newline", "Hello, world!", "Hello, world!\n");
    RUN_PUTENDL_FD_TEST("Write empty string", "", "\n");
    RUN_PUTENDL_FD_TEST("Write string with newline", "Line1\nLine2", "Line1\nLine2\n");
    RUN_PUTENDL_FD_TEST("Write string with special chars", "Tab\tEnd", "Tab\tEnd\n");

    printf("\n--- Test Summary for ft_putendl_fd ---\n");
