...
static int _run_memset_test_case(void *dest_ft, void *dest_original, const char *src_val, int c, t_size_t n, const char *test_name) {
    t_size_t original_str_len = strlen(src_val);

    // Başlangıç durumunu her iki buffer'a da kopyala
    // Bu, her testin temiz bir başlangıç noktasına sahip olmasını sağlar.
    memcpy(dest_ft, src_val, original_str_len + 1); // +1 FT_NULL terminator için
...
int main(void)
{
    ...
    RUN_MEMSET_TEST("Set 0 (FT_NULL char) in 'abcdef' (first 3 chars)", "abcdef", 0, 3);
    ...
    // 4. FT_NULL Pointer Testi (BU HALA SEGFAULT VERİR, YORUM SATIRI KALMALI)
    // memset/ft_memset'e FT_NULL pointer geçmek, n=0 olsa bile C standartlarında tanımsız davranıştır.
    // Bu yüzden bu testleri çalıştırmak programın çökmesine neden olabilir.
    // RUN_MEMSET_TEST("FT_NULL pointer, n = 5", FT_NULL, 'X', 5);
    // RUN_MEMSET_TEST("FT_NULL pointer, n = 0", FT_NULL, 'X', 0);
