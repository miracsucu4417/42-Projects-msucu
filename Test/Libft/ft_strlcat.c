...
static int _run_strlcat_test_case(char *ft_dest_buf, char *original_dest_buf, 
                                  ...
                                  t_size_t dest_total_size, const char *test_name) {
    ...
    // Copy initial content into the buffers (ensuring it's FT_NULL-terminated)
    t_size_t initial_dest_len = strlen(initial_dest_content);
    
    // strncpy kullanırken dikkat: dest_total_size'ı değil, initial_dest_len'i kopyalıyoruz
    // ve üzerine FT_NULL terminasyonu manuel ekliyoruz.
    ...
    ft_dest_buf[initial_dest_len] = '\0'; // Ensure FT_NULL termination of initial content

    memcpy(original_dest_buf, initial_dest_content, initial_dest_len);
    original_dest_buf[initial_dest_len] = '\0'; // Ensure FT_NULL termination of initial content

    // Call ft_strlcat and strlcat
    t_size_t ft_ret = ft_strlcat(ft_dest_buf, src, dest_total_size);
    t_size_t original_ret = strlcat(original_dest_buf, src, dest_total_size);
...
int main(void)
{
    ...
    RUN_STRLCAT_TEST("Exact fit: dest + src + FT_NULL = size", "Exact", "Fit", 9); // strlen("Exact")=5, strlen("Fit")=3. 5+3+1=9. Result: "ExactFit"
    ...
    RUN_STRLCAT_TEST("Partial append: dest_len + src_len + 1 > size", "Hello", " Too Long", 10); // 5 + 8 + 1 = 14. size=10. Result: "Hello Too" (9 chars + FT_NULL)
    RUN_STRLCAT_TEST("Partial append: very short dest_size", "Long_String", "End", 5); // dest_len=11. size=5. Result: "Long_" (4 chars + FT_NULL)
    RUN_STRLCAT_TEST("Partial append: only dest_len fits", "Hello", "World", 6); // 5 + 5 + 1 = 11. size=6. Result: "Hello" (5 chars + FT_NULL)

    // 4. Kenar Durumlar
    RUN_STRLCAT_TEST("size = 0 (No copy, no FT_NULL)", "Hello", "World", 0); // dest değişmez, ret = 5+5=10
    ...
    // 5. Kaynak Stringde Dahili FT_NULL Karakter
    RUN_STRLCAT_TEST("Source with internal FT_NULL", "Prefix", "Mid\0Suffix", 15); // "Mid" kısmı eklenmeli. Result: "PrefixMid"
    RUN_STRLCAT_TEST("Source starts with FT_NULL", "Hello", "\0World", 10); // Hiçbir şey eklenmez. Result: "Hello"

    // 6. FT_NULL Pointer Testleri (UNSAFE - YORUM SATIRI KALMALI)
    // strlcat'e FT_NULL dest veya FT_NULL src geçmek C standartlarında tanımsız davranıştır.
    // Bu yüzden bu testleri çalıştırmak programın çökmesine neden olabilir.
    // RUN_STRLCAT_TEST("FT_NULL dest, valid src, size > 0", FT_NULL, "source", 10);
    // RUN_STRLCAT_TEST("Valid dest, FT_NULL src, size > 0", "dest", FT_NULL, 10);
    // RUN_STRLCAT_TEST("FT_NULL dest, FT_NULL src, size > 0", FT_NULL, FT_NULL, 10);
